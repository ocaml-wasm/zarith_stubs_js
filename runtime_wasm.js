...
function wasm_z_normalize(z){
  return (z == BigInt.asIntN(31, z))?Number(z):z;
}
...
//Requires: wasm_z_div, wasm_z_add
function wasm_z_cdiv(z1, z2) {
  if (z1 == 0) return 0;
  if ((z1 > 0) == (z2 > 0)) /* Multiplication is like a signwise xor */ {
    if (BigInt(z1) % BigInt(z2) != 0) {
      return wasm_z_add(wasm_z_div(z1, z2), 1n);
    }
  }
  return wasm_z_div(z1, z2);
}

//Provides: wasm_z_fdiv
//Requires: wasm_z_div, wasm_z_sub
function wasm_z_fdiv(z1, z2) {
  if (z1 == 0) return 0;
  if ((z1 > 0) != (z2 > 0)) /* Multiplication is like a signwise xor */ {
    if (BigInt(z1) % BigInt(z2) != 0) {
      return wasm_z_sub(wasm_z_div(z1, z2), 1n);
    }
  }
  return wasm_z_div(z1, z2);
}
...
//Provides: wasm_z_gcd
//Requires: bigInt, wasm_z_normalize
function wasm_z_gcd(z1, z2) {
  return wasm_z_normalize(bigInt.gcd(bigInt(z1), bigInt(z2)).abs().value);
}
...
function wasm_z_powm(z1, z2, z3) {
  z3 = BigInt(z3);
  var r = bigInt(z1).modPow(z2, z3).value;
  if(r < 0) r += (z3 < 0)?(-z3):z3;
...
function wasm_z_probab_prime(z, i) {
  return +(bigInt(z).isProbablePrime(i))
}
...
function wasm_z_nextprime(z1) {
  ...
  while (true) {
    if (bigInt(z1).isProbablePrime(25)) {
      return wasm_z_normalize(z1);
    }
    z1 += 2n;
  }
}
...
//Provides: wasm_z_bin
//Requires: wasm_z_normalize, wasm_z_neg
function wasm_z_bin(n, k){
  ...
  if (n < 0) {
    ...
    if (k & 1n) coeff = wasm_z_neg(coeff);
    return coeff;
  }
  if (k > n) return 0;
  var k2 = n - k;
